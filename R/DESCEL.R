...
# perform DESeq2
Execute_DESeq2 <- function(object, DESeq2.test = "Wald", beta.Prior = TRUE, DESeq2.parallel = TRUE, DESeq2.fitType = "local",...){
  options(warn = -1)
...
DESCEL <- function(raw.count, cell.label,
                                     is.normalized = FALSE, verbose = TRUE,
                                     BPSC = TRUE, DEsingle = TRUE, DESeq2 = TRUE, 
                                     MAST = TRUE, monocle = TRUE, scDD = TRUE, Ttest = TRUE, Wilcoxon = TRUE,
                                     limma = TRUE,zingeR.edgeR = TRUE,
                                     BPSC.coef = 2, BPSC.normalize = "CPM", BPSC.parallel = TRUE,
                                     DEsingle.parallel = TRUE, DEsingle.normalize = "CPM",
                                     DESeq2.test = "LRT", DESeq2.parallel = TRUE, DESeq2.beta.prior = TRUE, DESeq2.fitType = "local", DESeq2.normalize = "CPM",
  ...
  Methods <- c("BPSC", "DEsingle", "DESeq2", "MAST", "monocle","scDD","Ttest", "Wilcoxon", "limma","zingeR.edgeR")
  Methods.idx <- c(BPSC,DEsingle,DESeq2,MAST,monocle,scDD,Ttest,Wilcoxon,limma,zingeR.edgeR)
...
spearman.matrix <- function(p.values){
  ...
  spearman.correlation <- (1+spearman.correlation)/2
  diag(spearman.correlation) <- 0
  return(spearman.correlation)
}

jaccard_index <- function(p.values){
  p.values <- as.matrix(p.values)
  methods.names <- colnames(p.values)
  jaccard.correlation <- matrix(0, nrow = ncol(p.values), ncol = ncol(p.values))
  rownames(jaccard.correlation) <- methods.names
  colnames(jaccard.correlation) <- methods.names
  for (i in 1:ncol(jaccard.correlation)) {
    index_smmoothing1 <- sort(p.values[,i],  decreasing = FALSE)
    gene_smoothing1 <- names(index_smmoothing1)[1:500]
    for (j in 1:ncol(jaccard.correlation)) {
      index_smmoothing2 <- sort(p.values[,j],  decreasing = FALSE)
      gene_smoothing2 <- names(index_smmoothing2)[1:500]
      jaccard.correlation[i,j] <- sum(gene_smoothing1 %in% gene_smoothing2) / length(union(gene_smoothing1, gene_smoothing2))
      #jaccard.correlation[i,j] <- jaccard::jaccard(gene_smoothing1, gene_smoothing2)
    }
  }
  diag(jaccard.correlation) <- 0
  return(jaccard.correlation)
} 

#'ACAT
p.ACAT <- function(Pvals, weights = TRUE){
  ...
  n.pval <- ncol(Pvals)
  if(weights){
    weight.matrix <- jaccard_index(p.values = Pvals)
    temp <- Matrix::rowSums(weight.matrix)/(ncol(weight.matrix)-1)
    weight <- matrix(rep(temp, n.gene), ncol = n.pval,byrow =TRUE)
  }else{
    weight <- matrix(rep(1.0/n.pval, n.pval * n.gene), ncol = n.pval)
  }
  if((nrow(weight) != n.gene) || (ncol(weight) != n.pval)){
  ...
  #combination.Pvals <- 1.0/2.0 - atan(Cbar)/pi
  combination.Pvals <- 1.0 - pcauchy(Cbar)
  combination.Pvals[which(combination.Pvals <= 0)] <- 5.55e-17
...
#'Lancaster
p.lancaster <- function(Pvals, weight = TRUE, trimmed = 0.2){
...
#'Fisher’s method
fisher.combination<-function(Pvals){
  object <- data.frame(Pvals)
  idx <- 1:nrow(object)
  names(idx) <- rownames(object)
  p.Fisher <- sapply(idx, function(i){
    sumlog(object[i,])$p
  })
  return(p.Fisher)
} 
...
#'Stouffer’s z-method
stouffer.combination<-function(Pvals){
  object<-as.matrix(Pvals)
  idx <- 1:nrow(object)
  names(idx) <- rownames(object)
  p.stouffer <- sapply(idx1, function(i){
    stouffer(object1[i,])$p
  })
  combination.Pvals<-p.stouffer
  return(combination.Pvals)
} 
#'weighted z-method
weighted.combination<-function(Pvals,weight = TRUE){
  #### check if there is NA
  if (sum(is.na(Pvals)) > 0){
    stop("Cannot have NAs in the p-values!")
  }
  ...
  idx <- 1:nrow(object)
  names(idx) <- rownames(object)
  p.weighted <- sapply(idx, function(i){
    sumz(object1[i,],weights=weight.pval)$p
  })
  combination.Pvals<-p.weighted
  return(combination.Pvals)
}


#'Mean
mean.combination<-function(Pvals){
  object <- data.frame(Pvals)
  idx <- 1:nrow(object)
  names(idx) <- rownames(object)
  p.mean <- sapply(idx, function(i){
    meanp(object[i,])$p
  })
  combination.Pvals<-p.mean
  return(combination.Pvals)
}
#'Vote
vote.combination<-function(Pvals){
  object <- data.frame(Pvals)
  idx <- 1:nrow(object)
  names(idx) <- rownames(object)
  p.vote <- sapply(idx, function(i){
    votep(object[i,])$p
  })
  combination.Pvals<-p.vote
  return(combination.Pvals)
}
...
#'
#' This function is used to adjust the p-values generated by individual DE analysis methods and DESCEL.
#'
...
DESCEL.p.adjust <- function(combination.Pvals, adjusted.method = "fdr"){
